/* ============================================= */
/* KI-Reifegrad-Assessment — Configuration        */
/* ============================================= */
...
// Collections (separate from team-assessments)
const SESSIONS_COLLECTION = 'maturity_sessions';
const RESPONSES_COLLECTION = 'maturity_responses';
...
function checkAdmin() {
  return sessionStorage.getItem('maturity_admin') === 'true';
}
function loginAdmin(password) {
  if (password === ADMIN_PASSWORD) {
    sessionStorage.setItem('maturity_admin', 'true');
    return true;
  }
  return false;
}
function logoutAdmin() {
  sessionStorage.removeItem('maturity_admin');
}
...
async function deleteSession(code) {
  if (!confirm(`Session ${code} und ALLE zugehörigen Antworten löschen?\n\nDies kann nicht rückgängig gemacht werden.`)) return;
  try {
    const snapshot = await db.collection(RESPONSES_COLLECTION)
    ...
    alert(`Session ${code} wurde erfolgreich gelöscht.`);
...
async function clearSessionResponses(code) {
  if (!confirm(`Alle Antworten für Session ${code} löschen?\n\nDie Session bleibt erhalten, aber alle Teilnehmerdaten werden entfernt.`)) return;
  try {
    const snapshot = await db.collection(RESPONSES_COLLECTION)
      .where('sessionCode', '==', code).get();
    if (snapshot.empty) { alert('Keine Antworten vorhanden.'); return; }
    const batch = db.batch();
    snapshot.forEach(doc => batch.delete(doc.ref));
    await batch.commit();
    alert(`${snapshot.size} Antwort(en) aus Session ${code} gelöscht.`);
    if (typeof loadDashboard === 'function') loadDashboard();
  } catch (err) {
    alert('Fehler beim Löschen der Antworten: ' + err.message);
    console.error(err);
  }
}
...
function exportMaturityCSV(sessionName, responses) {
  if (!responses.length) { alert('Keine Daten zum Exportieren.'); return; }
  ...
  const headers = ['Name', 'Email', 'Reifegrad-Level', ...questionIds.map(id => labelMap[id] || id), 'Gesamtscore', 'Datum'];
  ...
  const rows = responses.map(r => {
    ...
    return [
      ...
      ...answerValues,
      r.maturityScore || '',
      r.createdAt ? new Date(r.createdAt.seconds * 1000).toLocaleDateString('de-DE') : ''
    ];
  });
  ...
  a.href = url;
  a.download = `KI-Reifegrad_${sessionName.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
